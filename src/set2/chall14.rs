...
mod tests {
    ...
    struct UnknownWrapEncrypter { e: UnknownEncrypter, pre: Vec<u8>, }
    ...
    impl UnknownWrapEncrypter {
        ...
        fn new() -> UnknownWrapEncrypter {
            ...
            UnknownWrapEncrypter { e: enc, pre: pre[..pre_len].to_vec(), }
        }
    ...
    fn detect_prefix_size(
        enc: &mut UnknownWrapEncrypter,
        blk_size: usize
    ) -> Option<usize> {
        for i in 0..=blk_size {
            ...
            for j in 0..blks.len()-1 {
                if blks[j] == blks[j+1] {return Some(j*blk_size-i)}
            }
        }
        None
    }
